...
const AmenitiesHotelList = () => {
  ...
  const handleDelete = async (id) => {
    ...
    console.log(id);
      ...
      const response = await axios.delete(`http://localhost:3000/api/v1/hotel/deleteamenitieshotel/${id}`, {
        headers: {
  ...
  const handleEdit = async (record) => {
         navigate(`/amenities-hotel/edit/${record.id}`);
  ...
  return (
    <div className="p-6 w-full">
      <div className="flex justify-between items-center mb-6">
        <Title level={2} className="text-blue-800 m-0">Quản lý tiện nghi khách sạn</Title>
        <Button 
          type="primary" 
          icon={<PlusOutlined />} 
          className="bg-blue-500 hover:bg-blue-700"
          onClick={() => navigate('/amenities-hotel/add')}
        >
          Thêm tiện nghi khách sạn
        </Button>
      </div>
      ...
      <Table
        ...
        pagination={{
          ...
          showTotal: (total) => `Tổng cộng ${total} tiện nghi khách sạn`,
        }}
...
export default AmenitiesHotelList;
