...
const editAmenitiesHotel = () => {
    ...
    useEffect(() => {
        const fetchRoomAmenities = async () => {
            try {
                const response = await axios.get(`http://localhost:3000/api/v1/hotel/getamenitieshotelbyid/${id}`);
                const data = response.data.hotelAmenities;
    ...
    const onFinish = async (values) => {
        try {
            ...
            const response = await axios.put(`http://localhost:3000/api/v1/hotel/updateamenitieshotel/${id}`, formData, {
                headers: {
            ...
            if (response.data) {
                alert('Cập nhật tiện nghi khách sạn thành công!');
                navigate('/amenities-hotel');
            }
        } catch (error) {
            console.error('Lỗi khi cập nhật tiện nghi khách sạn:', error);
            message.error('Đã xảy ra lỗi khi cập nhật tiện nghi khách sạn. Vui lòng thử lại!');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="p-6 w-full">
            <div className="flex flex-col mb-6">
                ...
                <Title level={2} className="text-blue-800 m-0 mt-5">Cập nhật tiện nghi khách sạn mới</Title>
            </div>
...
export default editAmenitiesHotel;
